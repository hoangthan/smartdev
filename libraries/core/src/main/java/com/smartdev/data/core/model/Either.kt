 ...
 */
sealed class Either<out L : Throwable, out R> {
    /**
     * Represent for Error type
     */
    data class Left<out L : Throwable>(val err: L) : Either<L, Nothing>()
     ...
     */
    suspend fun <L : Throwable, R> Either<L, R>.onSuccess(
     ...
     */
    suspend fun <L : Throwable, R> Either<L, R>.onFailure(
     ...
     */
    fun <L : Throwable, R> Either<L, R>.get(value: R): R = (this as Right<R>).data

    /**
     * Get the data by casting the type as Right.
     * If the type is not right, the default value gonna be returned
     */
    fun <L : Throwable, R> Either<L, R>.getOrElse(value: R): R = when (this) {
        is Left -> value
        is Right -> data
    }
     ...
     */
    fun <L : Throwable, R> Either<L, R>.getOrNull(): R? = when (this) {
        is Left -> null
        is Right -> data
    }
     ...
     */
    suspend fun <T, L : Throwable, R> Either<L, R>.map(func: suspend (R) -> T): Either<L, T> =
        when (this) {
            is Left -> Left(err)
     ...
     * Unless, return the Left value as error
     */
    suspend fun <T, L : Throwable, R> Either<L, R>.flatMap(fn: suspend (R) -> Either<L, T>): Either<L, T> =
        when (this) {
            is Left -> Left(err)
            is Right -> fn(data)
        }
